...
const ServiceAccountsPage = () => {
  ...
  const [isServiceAccountDrawerOpen, setIsServiceAccountDrawerOpen] =
    useState(false);
      ...
      render: (rowData: User) => {
        ...
        return (
          <div className="flex gap-2 items-center">
            <code className="px-2 py-1 text-sm bg-gray-100 rounded font-mono">
              {masked}
            </code>
          </div>
        );
      ...
      render: (rowData) => {
        return (
          <span className="text-gray-600">
            {formatDateShort(
              rowData.tenant?.created_at || rowData.tenant?.updated_at || "",
            )}
          </span>
        );
      },
  ...
  return (
    <div>
      ...
      <ServiceAccountDrawer
        isOpen={isServiceAccountDrawerOpen}
        onOpenChange={setIsServiceAccountDrawerOpen}
      />
      ...
      {serviceAccountsResponse?.items.length === 0 && (
        <EmptyPage
          ...
          emptyStateCard={{
            ...
            description:
              "Create a service account to manage programmatic access with specific roles and permissions.",
