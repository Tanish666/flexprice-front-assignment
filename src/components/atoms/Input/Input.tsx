...
export const removeFormatting = (
  ...
  const escapedSeparator = thousandSeparator.replace(
    /[.*+?^${}()|[\]\\]/g,
    "\\$&",
  );
  ...
  switch (variant) {
    ...
    case "formatted-number":
      return allowNegative
        ? new RegExp(
            `^-?\\d*${allowDecimals ? `\\${decimalSeparator}?\\d*` : ""}$`,
          )
        : new RegExp(
            `^\\d*${allowDecimals ? `\\${decimalSeparator}?\\d*` : ""}$`,
          );
...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    const isFormattedVariant =
      variant === "formatted-number" || variant === "integer";
    const pattern = React.useMemo(
      () => getInputPattern(variant, formatOptions),
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (variant !== "text") {
        ...
        if (isFormattedVariant) {
          const oldFormatCharCount = (
            oldValue.slice(0, currentCursorPosition).match(/,/g) || []
          ).length;
          const newFormatCharCount = (
            formatAmount(newValue, formatOptions)
      ...
      <div className="space-y-1 w-full flex flex-col">
        ...
        <div
          className={cn(
            ...
            error
              ? "border-destructive"
              : "border-input focus-within:ring-ring focus-within:ring-offset-2",
        ...
        {description && (
          <p
            className={cn(
              "text-sm",
              disabled ? "text-zinc-500" : "text-muted-foreground",
            )}
          >
            {description}
          </p>
