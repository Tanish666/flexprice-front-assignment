  ...
  const handleTimezoneChange = useCallback(
    (newTz: CalendarTimezone) => {
      if (selectedRange?.from && selectedRange?.to) {
        const fromConverted = convertDateToTimezone(
          selectedRange.from,
          timezone as DateTimezone,
          newTz as DateTimezone,
        );
        const toConverted = convertDateToTimezone(
          selectedRange.to,
          timezone as DateTimezone,
          newTz as DateTimezone,
        );
      ...
      <PopoverTrigger className={popoverTriggerClassName} disabled={disabled}>
        <div className="flex flex-col ">
          {title && (
            <div
              className={cn(
                "text-sm font-medium mb-1 w-full text-start",
                labelClassName,
              )}
            >
              {title}
