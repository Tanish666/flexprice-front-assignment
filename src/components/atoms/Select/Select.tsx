  ...
  return (
    <div className={cn("space-y-1 ", className)}>
      ...
      {label && (
        <label
          className={cn(
            " block text-sm font-medium text-zinc break-words",
            disabled ? "text-zinc-500" : "text-zinc-950",
          )}
        >
          {label}
      ...
      >
        ...
        >
          {trigger ? (
            ...
            >
              {value
                ? options.find((option) => option.value === value)?.label.trim()
                : placeholder}
            </span>
          )}
        </SelectTrigger>
